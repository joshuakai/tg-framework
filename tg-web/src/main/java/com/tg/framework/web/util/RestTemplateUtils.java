...
public class RestTemplateUtils {
  ...
  public static boolean isQueryStringRequest(HttpMethod method) {
  ...
  public static <T> ResponseEntity<T> request(RestTemplate restTemplate, String url, String method,
      boolean isUsingPayload, boolean isTextPlain, Map<String, ?> parameters, Object payload,
    ...
    if (isQueryStringRequest(httpMethod)) {
      if (parameters != null && !parameters.isEmpty()) {
        parameters.forEach((k, v) -> Optional.ofNullable(v)
            .ifPresent(noneNull -> uriBuilder.queryParam(k, noneNull)));
      }
    } else if (isBodyRequest(httpMethod)) {
      if (isUsingPayload) {
        HttpHeaders headers = new HttpHeaders();
        if (payload instanceof String && isTextPlain) {
  ...
  public static ResponseEntity<String> request(RestTemplate restTemplate, String url, String method,
    ...
    return request(restTemplate, url, method, isUsingPayload, isTextPlain, parameters, payload,
        uriVariables, String.class);
  }

  public static SimpleResponseResult succeedOrFailedRequest(RestTemplate restTemplate, String url,
      String method, boolean isUsingPayload, boolean isTextPlain, Map<String, ?> parameters,
      Object payload, Map<String, Object> uriVariables) {
    ResponseEntity<String> responseEntity;
    try {
      responseEntity = request(restTemplate, url, method, isUsingPayload, isTextPlain, parameters,
          payload, uriVariables);
