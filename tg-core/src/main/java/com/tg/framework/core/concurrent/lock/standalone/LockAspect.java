...
import com.tg.framework.core.concurrent.lock.LockAspectSupport;
...
public class LockAspect extends LockAspectSupport<LockService> {
  ...
  @Around("@annotation(com.tg.framework.core.concurrent.lock.standalone.Lock)")
  ...
  protected LockContext getLockContext(ProceedingJoinPoint proceedingJoinPoint) {
    ...
    String key =
        lock.useExpression() ? getExpressionKey(lock.key(), method, proceedingJoinPoint.getArgs())
            : lock.key();
    long timeoutMillis =
